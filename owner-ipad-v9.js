document.addEventListener("DOMContentLoaded", () => {
  ...
  function actionButtons(order) {
    ...
    const fulfillment = order.fulfillment_type;

    if (["new", "reviewing", "quoted", "customer_waiting"].includes(status)) {
      return button(order, "confirmed", "注文確定", "btn-primary");
    }

    if (["confirmed", "payment_waiting", "production_waiting"].includes(status)) {
      return button(order, "producing", "制作開始", "btn-primary");
    }

    if (["producing", "completed"].includes(status)) {
      const nextStatus =
        fulfillment === "delivery"
          ? "delivery_preparing"
          : "pickup_waiting";
      return button(order, nextStatus, "制作完了", "btn-rose");
    }

    if (status === "pickup_waiting") {
      return button(order, "handed_over", "引渡し完了", "btn-primary");
    }

    if (status === "delivery_preparing") {
      return `<span class="notice">制作完了。スタッフ配達画面で出発処理を行います。</span>`;
    }

    if (status === "delivering") {
      return `<span class="notice">配達中です。</span>`;
    }

    if (status === "delivered") {
      return `<span class="badge">配達完了</span>`;
    }

    if (status === "handed_over") {
      return `<span class="badge">引渡し完了</span>`;
    }

    if (status === "cancelled") {
      return `<span class="badge rose">キャンセル</span>`;
    }

    return `<span class="help">現在実行できる操作はありません。</span>`;
  }

  function button(order, status, label, className = "btn-secondary") {
    return `<button class="btn ${className}" data-order-id="${DPRO.escape(order.id)}" data-status="${status}">${label}</button>`;
  }
});
